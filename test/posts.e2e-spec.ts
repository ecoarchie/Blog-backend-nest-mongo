...
describe('posts routes', () => {
  ...
  describe('GET "/posts/{id}"  - find post by ID', () => {
    it('should find post if id is valid', async () => {
      ...
      const postToFind = await request(app.getHttpServer()).get(`/posts/${createdPost.id}`).expect(200);
    ...
    it('should NOT find post with invalid ID', async () => {
      const result = await request(app.getHttpServer()).get(`/posts/ffjak`).expect(404);
    });
  });
